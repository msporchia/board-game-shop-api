...
describe('OrderRoutes', () => {
  ...
  it('POST builds the order from the cart, clears it, and GET lists the history', async () => {
    await app.inject({ method: 'PUT', url: '/carts/alice/items/1', payload: { quantity: 2 } });
    await app.inject({ method: 'PUT', url: '/carts/alice/items/5', payload: { quantity: 1 } });
    ...
    const cart = await app.inject({ method: 'GET', url: '/carts/alice' });
    expect(cart.json()).toEqual({ customerId: 'alice', items: [], totalCents: 0 });

    const history = await app.inject({ method: 'GET', url: '/orders?customerId=alice' });
  ...
  it('validates customerId on both routes', async () => {
    const badPost = await app.inject({
      method: 'POST',
      url: '/orders',
      payload: { customerId: 'spaces are bad' },
    });
    expect(badPost.statusCode).toBe(400);

    const badGet = await app.inject({ method: 'GET', url: '/orders?customerId=bad%20id' });
